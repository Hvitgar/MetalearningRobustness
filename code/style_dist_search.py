...
from augmentations import ParametricTransform, AdaptiveStyleTransfer
...
parser.add_argument('--temperature', type=float, default=8, help='set temperature for style selection')
...
parser.add_argument('--dummy-task', action='store_true', help='overwrites half of the styles with zeros, resulting in useless styletransfer')
parser.add_argument('--individual-alpha', action='store_true', help='learn individual magnitude parameter per style')
parser.add_argument('--disable-dist-learning', action='store_true', help='use uniform distribution over styles instead of learned one')

def main():
    best_acc1 = 0
    args = parser.parse_args()
    args.augmentations = ['Identity', 'AdaptiveStyleTransfer']
    ...
    if args.train:
        if args.augment_train_data:
            ...
            if 'AdaptiveStyleTransfer' in args.augmentations:
                ...
                augmodel.augmentations[1].temperature = torch.tensor(args.temperature, dtype=torch.float32)
                if args.individual_alpha:
                    augmodel.augmentations[1].initIndividualAlpha()
                    if args.disable_dist_learning:
                        augmodel.augmentations[1].logits.requires_grad = False
                if args.dummy_task:
                    augmodel.augmentations[1].style_features[:args.style_subset//2] -= augmodel.augmentations[1].style_features[:args.style_subset//2] # set first half of style features to zero, these produce close to uniform styletransferred images
...
def train(train_loader, val_loader, model, augmodel, criterion, ho_criterion, optimizer, ho_optimizer, epoch, args, writer):
    ...
    for i, batch in enumerate(train_loader):
        ...
        if args.ho and i % args.k == 0:
            ...
            with torch.no_grad():
                ...
                ast.logits.data = torch.nn.functional.log_softmax(ast.logits, dim=-1) # normalization
            model.train()
    ...
    if args.augment_train_data:
        ...
        for aug in augmodel.augmentations:
            if isinstance(aug, ParametricTransform):
                if isinstance(aug, AdaptiveStyleTransfer) and args.individual_alpha:
                    alphas = aug.mu_mag.detach().cpu().numpy()
                    individual_alphas = {str(i): val for i, val in enumerate(alphas)}
                    writer.add_scalars('AugNet/means', individual_alphas , epoch)
                else:
    ...
    if args.ho:
        ...
        if 'AdaptiveStyleTransfer' in args.augmentations:
            ...
            writer.add_scalars('AugNet/AST/style_logits', logits, epoch)
            writer.add_scalars('AugNet/AST/style_probs', probs, epoch)
