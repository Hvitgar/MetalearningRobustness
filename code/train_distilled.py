...
parser.add_argument('--style-indices', type=int, nargs='+', help='Select specific styles to use for style transfer')
        ...
        if args.augment_train_data:
            ...
            if 'AdaptiveStyleTransfer' in args.augmentations:
                ...
                if args.style_indices:
                    assert args.effective_bs % len(args.style_indices) == 0, "Number of style indices must be a divisor of effective bs!"
                    ast._PainterByNumbers = torch.utils.data.dataset.Subset(ast._PainterByNumbers, args.style_indices)
                    ast.initStyles(len(args.style_indices), seed=args.seed)
                    factor = args.effective_bs // len(args.style_indices)
                    ast.style_features = ast.style_features.repeat(factor, 1, 1, 1)
